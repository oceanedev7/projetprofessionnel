...
@section('title', __('content.nidDouillet'))
...
@endif
    ...
    <a href="{{ route('lang.switch', ['lang' => App::getLocale() === 'en' ? 'fr' : 'en']) }}" 
          ...
          <a href="{{ route('reserver') }}" class="absolute top-8 right-8 bg-custom-vert bg-opacity-90 tracking-widest text-white py-3 px-3 border-none rounded w-30 font-semibold text-sm uppercase"> {{ __('content.reserver') }} </a>  
</div>
@endsection
...
<div style="background-color:#F9F4EE" class="md:h-screen md:w-full md:flex md:flex-row flex flex-col space-y-4 md:space-y-0"> 
    ...
    <img class="w-[550px] md:h-screen" src="{{ Storage::url('images/cabanenid.jpg') }}" alt="Cabane intérieur">
